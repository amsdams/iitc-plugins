// ==UserScript==
// @id             iitc-plugin-coolclock@superd
// @name           IITC plugin: coolclock
// @category       Highlighter
// @version        0.0.0.1
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @updateURL      https://github.com/amsdams/iitc-plugins/raw/master/IITC_plugin_coolclock.meta.js
// @downloadURL    https://github.com/amsdams/iitc-plugins/raw/master/IITC_plugin_coolclock.user.js
// @description    [jonatkins-2013-06-12-162306] shows date time for current map position 
  ...
  window.plugin.coolClock.renderDialog = function () {
    ...
    $.get('https://maps.googleapis.com/maps/api/timezone/json', {
      ...
      function (json) {
        ...
        window.plugin.coolClock.displayBox(
          remoteDateTime, json.timeZoneId, json.timeZoneName, offsetInHours);
      }).fail(function (jqxhr, textStatus, error) {
      var err = textStatus + ', ' + error;
      console.log("Request Failed: " + err);
    });
  };
  window.plugin.coolClock.displayBox = function (date, timeZoneId,
    timeZoneName, offset) {
    var coolClockMenuContainer = $('<div>');
    coolClockMenuContainer.append($('<canvas id=\'clk2\' style=\'display:block;background-color:#FFF\' class=\'CoolClock:swissRail:138:1:' + (offset) + '\'></canvas>'));
    coolClockMenuContainer.append($('<span class=\"canvas_caption">' + date.toLocaleString() + '<br/>' + timeZoneId + '<br/>' + timeZoneName + '</span>'));
    var dialog = $('#dialog-coolclock');
    dialog.html(coolClockMenuContainer.clone().wrap('<div>').parent());
